...
namespace Gamekit2D
{
    ...
    public class InventoryItem : MonoBehaviour, IDataPersister
    {
        ...
        public GameObject SpriteIconPrefab;
        ...
        {
            if (layers.Contains(other.gameObject) && other.CompareTag("Player"))
            {
                var ic = other.GetComponent<CollectionManager>();

                if (ic.InventoryFull() && inventoryKey != "Slime")
                    return;
